...
public:
  ...
  void tick(uint64_t ts, uint32_t& seq, float level_pct) {
...
public:
  using DeviceBase::DeviceBase;
  void tick(uint64_t ts, uint32_t& seq, float gas_pct) {
...
public:
  using DeviceBase::DeviceBase;
  void tick(uint64_t ts, uint32_t& seq, float deg) {
...
TEST(DeviceBase, WaterTank_PublishesAllowedMetrics) {
  ...
  uint32_t seq = 0;
  dev.tick(/*ts*/1000, seq, /*level_pct*/67.0f);
